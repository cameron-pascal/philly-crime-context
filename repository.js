...
function getAllCensusDataWithFilters(req, res, next) {
//start time
  var startTime = req.query.startTime;
  //end time
  var endTime = req.query.endTime;
...
//list of GIDS
  ...
  */
  ...
  var finquery = 'SELECT dc_number as dcNum, census_ref as censusref, dispatch_date_time as timeOfCrime, general_crime_category as crime, w.tmax as maxTemp, w.tmin as minTemp from philly_crime_incidents, philly_weather w where to_timestamp(' + startTime + ') <= dispatch_date_time AND dispatch_date_time <= to_timestamp('+endTime+') AND ('+ crimeQuery +') AND ('+ con +') AND ('+ dorn +') AND w.begintime = weather_ref LIMIT 1000'
...
module.exports = {
  ...
  getAllCensusDataWithFilters: getAllCensusDataWithFilters
};
